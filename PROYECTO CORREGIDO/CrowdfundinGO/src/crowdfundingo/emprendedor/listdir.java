 ...
 * @author Henry Ruiz
 */
public class listdir extends javax.swing.JFrame {
    ...
    public void llenarTablaProy() throws SQLException, ClassNotFoundException {
        ...
        s.executeQuery("SELECT ID, TITULO, DINERO FROM PROYECTO WHERE ID_EMP=" + globalvars.id);
        ResultSet rs = s.getResultSet();
        System.out.println(rs);
        ProyTables.setModel(DbUtils.resultSetToTableModel(rs));
        Statement ss = con.createStatement();
        ss.executeQuery("SELECT SUM(DINERO) AS SUMATOTAL FROM PROYECTO WHERE ID_EMP=" + globalvars.id);
        ResultSet rss = ss.getResultSet();
        System.out.println(rss);
        while (rss.next()) {
            if(rss.getString("SUMATOTAL")==null){
                total.setText("No tiene nigún financiamiento");
            }else{
                total.setText("Total Recaudado: "+rss.getString("SUMATOTAL")+"$");
            }
        }
    }
    ...
    private void formWindowOpened(java.awt.event.WindowEvent evt) {//GEN-FIRST:event_formWindowOpened
        try {
            ...
            Logger.getLogger(listdir.class.getName()).log(Level.SEVERE, null, ex);
        }
